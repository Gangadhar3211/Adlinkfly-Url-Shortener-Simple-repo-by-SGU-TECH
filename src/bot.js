...
bot.onText(/\/start/, (msg) => {
  ...
  const welcomeMessage = `😇 Hello, ${username}!\n\n`
    ...
    + 'How To Use Me 👇👇 \n\n'
  + '✅1. Got To https://nglinks.rf.gd & Complete Your Registration.\n\n'
  + '✅2. Then Copy Your API Key from here https://nglinks.rf.gd/member/tools/api Copy Your API Only. \n\n'
...
bot.onText(/\/setapi (.+)/, (msg, match) => {
  ...
  // Save the user's NG Links API token to the database
...
async function shortenUrl(chatId, url) {
  ...
  try {
    const apiUrl = `https://nglinks.rf.gd/api?api=${nglinksToken}&url=${encodeURIComponent(url)}`;
